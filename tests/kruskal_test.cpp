...
static void StarGraphTest(httplib::Client* cli);
...
void TestKruskal(httplib::Client* cli) {
  ...
  RUN_TEST_REMOTE(suite, cli, StarGraphTest);
...
/**
 * @brief Тест на граф-звезду.
 *
 * @param cli Указатель на HTTP клиент.
 *
 * Граф: центральная вершина 100 соединена с вершинами 200, 300, 400.
 * Остовное дерево совпадает со всем графом, суммарный вес = 6.
 */
static void StarGraphTest(httplib::Client* cli) {
  nlohmann::json input;
  input["graph_type"] = "WeightedGraph";
  input["weight_type"] = "int";
  input["vertices"] = std::vector<size_t>{100, 200, 300, 400};
  input["edges"][0]["from"] = 100;
  input["edges"][0]["to"] = 200;
  input["edges"][0]["weight"] = 1;
  input["edges"][1]["from"] = 100;
  input["edges"][1]["to"] = 300;
  input["edges"][1]["weight"] = 2;
  input["edges"][2]["from"] = 100;
  input["edges"][2]["to"] = 400;
  input["edges"][2]["weight"] = 3;
